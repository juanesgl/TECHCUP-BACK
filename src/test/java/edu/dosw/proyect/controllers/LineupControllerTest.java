...
class LineupControllerTest {
    ...
    private OpponentLineupResponseDTO buildDTO(String nombreRival) {
        return OpponentLineupResponseDTO.builder()
                .partidoId(1L)
    ...
    void consultarAlineacionRival_HappyPath_LocalConsultaVisitante() {
        OpponentLineupResponseDTO dto = buildDTO("Equipo Beta");
        when(lineupService.consultarAlineacionRival(1L, 1L)).thenReturn(dto);

        ResponseEntity<OpponentLineupResponseDTO> response =
                lineupController.consultarAlineacionRival(1L, 1L);
        ...
        verify(lineupService, times(1))
                .consultarAlineacionRival(1L, 1L);
    }

    @Test
    void consultarAlineacionRival_HappyPath_VisitanteConsultaLocal() {
        OpponentLineupResponseDTO dto = buildDTO("Equipo Alpha");
        when(lineupService.consultarAlineacionRival(1L, 2L)).thenReturn(dto);

        ResponseEntity<OpponentLineupResponseDTO> response =
                lineupController.consultarAlineacionRival(1L, 2L);
